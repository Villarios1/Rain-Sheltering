...
M.isNpcNearPoint = function(npc, point)
    log:debug("Distance to point is: %s, configMax: %s", getDistanceFromPositionToPoint(npc.position, point), config.arriveDistance)
...
M.sendNpcToPoint = function(npc, point)
    if not npc.mobile then
        log:debug("Bug in sendNpcToPoint. NPC has no mobile! %s", npc.id)
    end
    tes3.setAITravel({
...
local function cancelTravelPackage(npc)
    if tes3.getCurrentAIPackageId({ reference = npc }) ~= tes3.aiPackage.travel then
        log:debug("Bug in cancelTravelPackage. Current package is not travel! %s", tes3.getCurrentAIPackageId({ reference = npc }))
        return
    end
    ...
    if not npc.mobile then
        log:debug("Bug in cancelTravelPackage. NPC has no mobile! %s", npc.id)
    end
    log:debug("cancelTravelPackage: %s", npc.id)
    ...
    timer.start({
        ...
        callback = function()
            ...
            if currentStep == steps and callback then
                log:debug("rotation is over. Calling back...")
                callback()
            end
        end
    })
...
M.sendNpcToNextPoint = function(npc, data)
    log:debug("NPC %s is at an intermediate key point.", npc.id)
    cancelTravelPackage(npc)
    data.nextPoint = data.nextPoint + 1
    log:debug("Sending to nextPoint: %s / %s", data.nextPoint, #data.keyPoints)
    M.sendNpcToPoint(npc, data.keyPoints[data.nextPoint])
    log:debug("Sended to point: %s", data.keyPoints[data.nextPoint])
end
...
local function buildRouteAcrossRiver(origin, cellKey, shelterPoint)
    ...
    if originSide ~= shelterSide then
        ...
        local bridgeIdx = 1
        log:debug("searchBank[1]: %s", searchBank[1])
        ...
        log:debug("npcSide: %s, shelterSide: %s", originSide, shelterSide)
        log:debug("onBridge: %s, bridgeIdx: %s", onBridge, bridgeIdx)
        -- 3. Формируем маршрут
        if onBridge then
        ...
        log:debug("The route across the river was built.")
        return keyPoints
    end
end
...
M.buildRoute = function(npc, cellKey, endPoint)
    ...
    if not originZonePoints
        ...
        log:debug("buildRoute. Direction is not routed. Setted to endpoind.")
    ...
    log:debug("isGoingBack: %s, endPoint: %s", isGoingBack, endPoint)
    ...
    log:debug("Route is builded. Total keypoints received: %s. nextPoint: %s", #data.keyPoints, data.nextPoint)
end
