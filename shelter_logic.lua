...
local function findNearestFreeShelter(reference, shelters, cellKey)
	...
	log:debug("nearestDistance found: %s", nearestDistance)
	return nearest
end
...
local function sendNpcToShelter(npc, shelterPoint, cellKey)
	log:debug("\nSending new NPC: %s: to: %s", npc.id, shelterPoint.name)
	...
	log:debug("Keypoints: %s / %s", npcData.getNpcData(npc).nextPoint, #npcData.getNpcData(npc).keyPoints)
end
...
local function checkIfInCombat(npc, data)
	if data.inCombat then
		...
		log:debug("Combat stopped. Resuming travel.")
	end
end
...
local function onArriveAtShelter(npc, data)
	...
	log:debug("NPC is at shelter, and turned 180: %s -> %s", npc.id, data.shelterName)
end
...
local function stayAtShelter(npc, data)
	...
	if npc.mobile.inCombat then
		...
		log:debug("NPC left shelter for combat. Will return after combat.")
	end
end

local function restoreOriginBehavior(npc, data)
	if not npc.mobile then
		log:debug("Bug in restoreOriginBehavior. NPC has no mobile! %s", npc.id)
	end
	log:debug("restoreOriginBehavior: %s", npc.id)
	tes3.setAIWander({
...
local function releaseShelter(cellKey, shelterName, npcId)
	...
	log:debug("releaseShelter. Shelter will free: %s", shelterName)
...
local function onReturningOrigin(npc, data)
	...
	log:debug("NPC returned and resumed package: %s", npc.id)
end

local function updateShelteringNpc(npc, data)
	log:debug("\nUpdating NPC: %s:", npc.id)
	data.lastSeenTime = getCurrentTime()
    log:debug("NPC is in combat: %s", npc.mobile.inCombat)
	log:debug("data.inCombat: %s", data.inCombat)
	tes3.messageBox("Updating")
...
local function beginNpcReturn(npc, data)
	log:debug("beginNpcReturn, %s", npc.id)
	...
	if data.phase == "atShelter" then
		...
		log:debug("Returning NPC to origin: %s (from %s)", npc.id, data.shelterName)
	else --"toShelter" Если дождь закончился пока NPC шел в укрытие
		-- нужно обновить nextPoint для новопостроенного маршрута
		log:debug("Rain is over while npc is going to shelter. Returning.")
		log:debug("Before: nextPoint: %s / %s", data.nextPoint, #data.keyPoints)
		data.nextPoint = #data.keyPoints - data.nextPoint
		log:debug("After: nextPoint: %s / %s", data.nextPoint, #data.keyPoints)
		navigation.sendNpcToNextPoint(npc, data)
	end
...
local function backToShelter(npc, cellKey, data)
	...
	-- нужно обновить nextPoint для новопостроенного маршрута
	log:debug("Rain is starts while npc is going to origin. Returning to shelter.")
	log:debug("Before: nextPoint: %s / %s", data.nextPoint, #data.keyPoints)
	data.nextPoint = #data.keyPoints - data.nextPoint
	log:debug("After: nextPoint: %s / %s", data.nextPoint, #data.keyPoints)
	navigation.sendNpcToNextPoint(npc, data)
...
local function processNpcSheltering(npc, shelters, cellKey)
	log:debug("\nprocess. Iterating next npc: %s", npc.id)
	...
	if package == tes3.aiPackage.none then -- у мертвых тоже none, но их надо выписывать
		log:debug("Package is none, npc: %s", npc.id)
		return
	end

	local data = npcData.getNpcData(npc)
	if data then
		log:debug("phase: %s", data.phase)
		log:debug("shelterName: %s", data.shelterName)
		log:debug("range: %s", data.range)
		log:debug("nextPoint: %s", data.nextPoint)
	end
	...
	if data == nil then -- пытаемся найти укрытие для NPC, и отправем его в него
		...
		if package == tes3.aiPackage.travel then log:debug("travel npc") return end
	...
	log:debug("Processing npc is over. Checking for next NPC...\n")
end
...
local function processSheltering()
	...
	for cell, _ in pairs(state.cellsToSheltering) do
		...
		if not isExteriorCell(cell) then log:debug("Bug in processSheltering! Not exterior!") return end
		...
		if #shelters == 0 then log:debug("Bug in processSheltering! No shelters!") return end
...
local function releaseNpcFromShelter(npc)
	log:debug("\nReleasing new NPC from Shelter: %s", npc.id)
	local data = npcData.getNpcData(npc)
	if not data then return end
	log:debug("phase: %s", data.phase)

	if not isValidNpc(npc) then
		log:debug("Bug in releaseNpcFromShelter. Clearing cause invalid NPC: %s", npc.id)
		clearNpcShelterState(npc)
		return
	end

	local package = tes3.getCurrentAIPackageId({ reference = npc })
	log:debug("package: %s", tes3.getCurrentAIPackageId({ reference = npc }))
...
local function releaseAllShelters()
	log:debug("\nreleaseAllShelters. state.isRaining: %s", state.isRaining)
	for cell, _ in pairs(state.cellsToSheltering) do
		if not isExteriorCell(cell) then log:debug("Bug in processSheltering! Not exterior!") return end
...
local function resetNpc(npc, data)
	...
	clearNpcShelterState(npc)
	log:debug("NPC teleported back and resumed package.")
end
...
local function onTimeSkip(e)
    ...
    for _, cell in ipairs(tes3.getActiveCells()) do
        if canCellBeSheltered(cell) then
            for npc in cell:iterateReferences(tes3.objectType.npc) do
                log:debug("Elapsed. Checking npc: %s", npc.id)
                local data = npcData.getNpcData(npc)
                if data then
                    resetNpc(npc, data)
                else
                    log:debug("Bug in onRestWaitMenu. Npc has no data: %s", npc.id)
                end
            end
        end
    end
...
local function resetNpcIfLongAbsence(npc, data, currentTime)
	...
	log:debug("resetNpcIfLongAbsence. Npc: %s", npc.id)
	...
	if data and data.lastSeenTime then
		...
		log:debug("Bug2. longAbsence npc have no data!")
	end
end
...
local function resetActiveCells()
	...
	log:debug("Loaded. Cells has been reseted.")
end
